# Historical README: prototypes and diagnostics

Archived from revision `8db2c48` on 20 September 2026. This preserves earlier
Count5/Count7 experiments and diagnostic instructions; it is not the current
public demo guide. Status statements below belong to that historical snapshot.
Commands assume the repository root, not this `docs` directory.

Start with the [current README](../README.md) or the
[current architecture and handoff](ARCHITECTURE_AND_HANDOFF.md).
...
For collaborators, start with the [architecture and co-hacker handoff](ARCHITECTURE_AND_HANDOFF.md): current Mermaid diagrams, code map, proof and payment flows, trust boundaries, verified evidence, and the Scale-track demo checklist.
...
pending.** See [deployment evidence and the acceptance checklist](SEP_ANCHOR_DEPLOYMENT.md).
The [release review](SEP_ANCHOR_REVIEW.md) records resolved findings,
...
See [the interoperability profile](SEP_ZKPASSPORT_INTEROPERABILITY.md) and
...
See [the fixture compatibility findings](SYNTHETIC_DOCUMENT_SETUP.md).
...
See [the implemented API and recovery model](ANCHOR_GATE_INTEGRATION.md),
[contract rules](GATE_VAULT_DESIGN.md), and
[deployment evidence](TESTNET_GATE_DEPLOYMENT.md). A public hostname needs
...
Do not use direct Node-created SDK sessions for this phone flow. The production relay reported their origin as `nodejs`, which the reviewed mobile origin check rejected for `localhost`, even with developer mode enabled. A genuine browser-origin request fixed the observed pre-Verify failure; origin checks were not weakened or spoofed. See [phone diagnostics](PHONE_DIAGNOSTICS.md) for the diagnosis and successful run.
