/**
 * Legacy SEP-12 KYC is simulated. Native diagnostic mode never reports acceptance.
 * Optional contact fields may be stored; identity document fields are dropped.
 */
...
const OPTIONAL_FIELDS: Record<string, { type: string; description: string }> = {
  first_name: {
    type: "string",
    description: "First name (optional in this sandbox)",
  },
  last_name: {
    type: "string",
    description: "Last name (optional in this sandbox)",
  },
  email_address: {
    type: "string",
    description: "Email (optional in this sandbox)",
  },
  bank_account_number: {
    type: "string",
    description:
      "Turkish IBAN for TRY payouts (optional; a sandbox IBAN is used otherwise)",
  },
...
export function sep12Routes(deps: Deps, sep: SepContext) {
  ...
  const customerOut = (c: CustomerRow) => {
    ...
    if (c.iban)
      provided.bank_account_number = {
        ...OPTIONAL_FIELDS.bank_account_number,
        status: fieldStatus,
      };
    const fields: Record<string, unknown> = {};
    for (const [k, v] of Object.entries(OPTIONAL_FIELDS))
      if (!(k in provided)) fields[k] = { ...v, optional: true };
    const registered = !!c.sep12_registered;
    return {
      // SEP-12: a customer the anchor has not collected anything for has no id yet.
      ...(registered ? { id: c.id } : {}),
      status: legacy && registered ? "ACCEPTED" : "NEEDS_INFO",
      message: !legacy
        ? POLICY_PENDING_MESSAGE
        : registered
  ...
  function resolve(c: Context<SepEnv>): CustomerRow | null {
    ...
    if (!legacy) {
      const [account, memo = ""] = c.get("sepSub").split(":");
      if (id && id !== me.id) return null;
      if (
        c.req.query("account") !== undefined &&
        c.req.query("account") !== account
      )
        return null;
      if (c.req.query("memo") !== undefined && c.req.query("memo") !== memo)
        return null;
      return me;
    }
    if (id) {
      const row = db
        .prepare("SELECT * FROM customers WHERE id = ? AND partner_id = ?")
        .get(id, me.partner_id) as unknown as CustomerRow | undefined;
      return row ?? null;
    }
  ...
  app.put("/sep12/customer", async (c) => {
    ...
    if (!legacy) {
      row = c.get("sepCustomer");
      const [account, memo = ""] = c.get("sepSub").split(":");
      if (
        (body.id !== undefined && body.id !== row.id) ||
        (body.account !== undefined && body.account !== account) ||
        (body.memo !== undefined && body.memo !== memo)
      ) {
        return sepError(c, 404, "customer not found");
      }
    } else if (typeof body.id === "string") {
      row =
        (db
          .prepare("SELECT * FROM customers WHERE id = ? AND partner_id = ?")
          .get(body.id, c.get("sepCustomer").partner_id) as unknown as
          CustomerRow | undefined) ?? null;
      if (!row) return sepError(c, 404, "customer not found");
    } else {
      const base =
        typeof body.account === "string" && body.account
          ? body.account
          : c.get("sepSub").split(":")[0]!;
      const memo =
        typeof body.memo === "string" && body.memo
          ? body.memo
          : c.get("sepSub").split(":")[1];
      row = ensureSepCustomer(
        db,
        memo ? `${base}:${memo}` : base,
        deps.cfg.anchorMode
      );
    ...
    db.prepare(
      "UPDATE customers SET first_name = ?, last_name = ?, email = ?, iban = ?, sep12_registered = 1, updated_at = ? WHERE id = ?"
    ).run(first, last, email, iban, nowIso(), row.id);
  ...
  app.put("/sep12/customer/callback", async (c) => {
    ...
    try {
      const body = (c.req.header("content-type") ?? "").includes(
        "application/json"
      )
        ? ((await c.req.json()) as Record<string, unknown>)
        : ((await c.req.parseBody()) as Record<string, unknown>);
    ...
    db.prepare(
      "UPDATE customers SET kyc_callback_url = ?, updated_at = ? WHERE id = ?"
    ).run(url, nowIso(), c.get("sepCustomer").id);
    return c.json({});
  });
  ...
  app.delete("/sep12/customer/:account", async (c) => {
    ...
    if (account !== me.split(":")[0])
      return c.json(
        { error: "you can only delete your own customer records" },
        404
      );
    ...
    try {
      ...
      if (body) {
        const parsed = (c.req.header("content-type") ?? "").includes(
          "application/json"
        )
          ? JSON.parse(body)
    ...
    if (!legacy && memo !== undefined && memo !== (me.split(":")[1] ?? "")) {
      return sepError(c, 404, "customer not found");
    }
    const sub = !legacy
      ? me
    ...
    const row = db
      .prepare(
        "SELECT * FROM customers WHERE partner_id = ? AND external_id = ?"
      )
      .get(c.get("sepCustomer").partner_id, sub) as unknown as
      CustomerRow | undefined;
    if (!row) return sepError(c, 404, "customer not found");
    // Forget everything optional the wallet sent; the balance/ledger history stays for the sandbox.
    db.prepare(
      "UPDATE customers SET first_name = 'Wallet', last_name = ?, email = NULL, kyc_callback_url = NULL, sep12_registered = 0, updated_at = ? WHERE id = ?"
    ).run(`${sub.slice(0, 4)}...${sub.slice(-4)}`, nowIso(), row.id);
    return c.json({});
  });

  app.all("/sep12/customer/files", (c) =>
    c.json({ error: "file uploads are not supported in this sandbox" }, 404)
  );
