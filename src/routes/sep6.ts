...
export function sep6Routes(deps: Deps, sep: SepContext) {
  ...
  app.get("/sep6/info", (c) => {
    const asset: Record<string, unknown> = {
      enabled: economicActionsEnabled(cfg),
      authentication_required: true,
      fee_percent: feePercent,
      funding_methods: [FUNDING_METHOD],
    };
    // Advertise limits only when configured. Omitted means no limit (hackathon default).
    if (Number(cfg.minOnrampTry) > 0)
      asset.min_amount = Number(cfg.minOnrampTry);
    if (Number(cfg.maxOnrampTry) > 0)
      asset.max_amount = Number(cfg.maxOnrampTry);
    ...
    const depositFields = {
      type: {
        description:
          "How the TRY arrives. Only bank_account (bank transfer) is supported.",
    ...
    return c.json({
      ...
      "deposit-exchange": {
        [stellar.assetCode]: { ...asset, fields: depositFields },
      },
      withdraw: {
        [stellar.assetCode]: {
          ...asset,
          types: { [FUNDING_METHOD]: { fields: {} } },
        },
      },
      "withdraw-exchange": {
        [stellar.assetCode]: {
      ...
      fee: {
        enabled: false,
        description:
          "Fee is a flat spread over the USD/TRY mid rate; see /sep38/price or the fee fields on each transaction.",
      },
  ...
  app.get("/sep6/tx/:id", (c) => {
    ...
    if (!b)
      return c.html(
        page("Transaction not found", "<p>No such transaction.</p>"),
        404
      );
    return c.html(txPage(b));
  });
  app.post("/sep6/tx/:id/simulate-bank-transfer", async (c) => {
    assertEconomicActionsEnabled(cfg);
    const b = loadSepTx(db, c.req.param("id"));
    ...
    try {
      ...
      if (accept.includes("text/html"))
        return c.redirect(`/sep6/tx/${b.tx.id}`);
      return c.json({
        ok: true,
        transaction: sepTransactionOut(cfg, stellar, updated),
      });
    } catch (e) {
  ...
  app.use("/sep6/*", async (c, next) => {
    if (c.req.path.startsWith("/sep6/tx/") || c.req.path === "/sep6/info")
      return next();
  ...
  function parseCommon(c: Context<SepEnv>, onchainAssetParam: string) {
    ...
    if (assetCode !== stellar.assetCode)
      return {
        error: `unsupported ${onchainAssetParam} '${assetCode}'; this anchor ramps ${stellar.assetCode}`,
      };
    const funding = q("funding_method") ?? q("type");
    if (funding && funding !== FUNDING_METHOD)
      return {
        error: `unsupported funding_method '${funding}'; use ${FUNDING_METHOD}`,
      };
    ...
    if (
      ...
      return { error: "'on_change_callback' must be a URL" };
    return { assetCode, funding: funding ?? FUNDING_METHOD, lang, callback };
  }

  function insertSepTx(
    row: Omit<
      SepTransactionRow,
      | "created_at"
      | "updated_at"
    ...
    db.prepare(
      ...
      row.quote_id,
      row.funding_method,
      row.claimable_balance_supported,
      row.on_change_callback,
      row.lang,
      row.reference,
      row.refund_memo,
      row.refund_memo_type,
      row.onramp_id,
      row.offramp_id,
      ts,
      ts
  ...
  async function deposit(c: Context<SepEnv>, exchange: boolean) {
    ...
    const common = parseCommon(
      c,
      exchange ? "destination_asset" : "asset_code"
    );
    ...
    if (
      ...
      !StrKey.isValidMed25519PublicKey(account)
    ) {
      return sepError(
        c,
        400,
        "'account' must be a Stellar G... or M... address (contract addresses are not supported)"
      );
    }
    const memoType = q("memo_type") ?? null;
    const memo = q("memo") ?? null;
    if (memo && memoType && !["text", "id", "hash"].includes(memoType))
      return sepError(c, 400, "'memo_type' must be text, id or hash");
    ...
    if (exchange) {
      ...
      if (q("source_asset") !== TRY_ASSET)
        return sepError(c, 400, `'source_asset' must be ${TRY_ASSET}`);
      if (!q("amount"))
        return sepError(c, 400, "'amount' is required for /deposit-exchange");
      if (q("quote_id")) {
        quote =
          (db
            .prepare("SELECT * FROM quotes WHERE id = ? AND customer_id = ?")
            .get(q("quote_id")!, customer.id) as unknown as
            QuoteRow | undefined) ?? null;
        if (!quote)
          return sepError(c, 400, "quote_id not found for this account");
        if (quote.side !== "buy")
          return sepError(
            c,
            400,
            "quote_id must sell TRY and buy USDC for a deposit"
          );
        if (quote.consumed_by)
          return sepError(c, 400, "quote_id was already used");
        if (new Date(quote.expires_at).getTime() < Date.now())
          return sepError(c, 400, "quote_id has expired");
        if (quote.source_amount !== fmtTry(parseTry(q("amount")!)))
          return sepError(
            c,
            400,
            `amount must equal the quote's sell_amount (${quote.source_amount})`
          );
      }
    }
    if (q("amount")) {
      try {
        ...
        if (Number(cfg.minOnrampTry) > 0 && kurus < parseTry(cfg.minOnrampTry))
          return sepError(
            c,
            400,
            `amount below minimum (${cfg.minOnrampTry} TRY)`
          );
        if (Number(cfg.maxOnrampTry) > 0 && kurus > parseTry(cfg.maxOnrampTry))
          return sepError(
            c,
            400,
            `amount above maximum (${cfg.maxOnrampTry} TRY)`
          );
        amount = fmtTry(kurus);
      } catch (e) {
    ...
    insertSepTx({
      ...
      claimable_balance_supported:
        q("claimable_balance_supported") === "true" ? 1 : 0,
    ...
    return c.json({
      ...
      eta: 5,
      ...(Number(cfg.minOnrampTry) > 0
        ? { min_amount: Number(cfg.minOnrampTry) }
        : {}),
      ...(Number(cfg.maxOnrampTry) > 0
  ...
  async function fundSepDeposit(
    ...
    if (b.tx.kind !== "deposit")
      throw new ApiError(
        400,
        "not_a_deposit",
        "Only deposits receive bank transfers"
      );
    if (b.tx.onramp_id)
      throw new ApiError(
        409,
        "already_funded",
        "This deposit already received its bank transfer"
      );
    if (b.tx.status_override)
      throw new ApiError(
        409,
        "not_pending",
        `Deposit is ${b.tx.status_override}`
      );
    const kurus = parseTry(
      amount ?? b.tx.amount_expected ?? "1000.00",
      "amount"
    );
    const customer = db
      ...
      .get(
        b.tx.customer_id
      ) as unknown as import("../core/types.js").CustomerRow;
    let quote: QuoteRow | null = null;
    if (b.tx.quote_id) {
      const qrow = db
        .prepare("SELECT * FROM quotes WHERE id = ?")
        .get(b.tx.quote_id) as unknown as QuoteRow | undefined;
      if (
        qrow &&
        !qrow.consumed_by &&
        new Date(qrow.expires_at).getTime() >= Date.now() &&
        qrow.source_amount === fmtTry(kurus)
      )
        quote = qrow;
    }
    ...
    tx(db, () => {
      ...
      const note =
        b.tx.quote_id && !quote
          ? "Quote expired or amount differed; converted at the live rate."
          : null;
      db.prepare(
        "UPDATE sep_transactions SET onramp_id = ?, message = ?, updated_at = ? WHERE id = ?"
      ).run(onramp.id, note, ts, b.tx.id);
    });
  ...
  async function withdraw(c: Context<SepEnv>, exchange: boolean) {
    ...
    if (exchange) {
      ...
      if (q("destination_asset") !== TRY_ASSET)
        return sepError(c, 400, `'destination_asset' must be ${TRY_ASSET}`);
      if (!q("amount"))
        return sepError(c, 400, "'amount' is required for /withdraw-exchange");
      if (q("quote_id")) {
        quote =
          (db
            .prepare("SELECT * FROM quotes WHERE id = ? AND customer_id = ?")
        ...
        if (quote.side !== "sell")
          return sepError(
            c,
            400,
            "quote_id must sell USDC and buy TRY for a withdrawal"
          );
        if (quote.consumed_by)
          return sepError(c, 400, "quote_id was already used");
        if (new Date(quote.expires_at).getTime() < Date.now())
    ...
    if (
      refundMemo &&
      refundMemoType &&
      !["text", "id", "hash"].includes(refundMemoType)
    )
      return sepError(c, 400, "'refund_memo_type' must be text, id or hash");

    try {
      const rate = await resolveRate(deps, "sell", quote);
      const id = newId("sep");
      const offramp = tx(db, () => {
        const row = createOfframp(deps, customer, {
          expected,
          rate,
          autoPayout: true,
      ...
      return c.json({
        ...
        extra_info: {
          ...
          payment_uri: stellarPayUri({
            destination: offramp.deposit_address,
            assetCode: stellar.assetCode,
            assetIssuer: stellar.assetIssuer,
            memoId: offramp.memo_id,
            amount: offramp.expected_usdc,
            msg: "TR Mock Anchor withdrawal",
          }),
        },
      });
    } catch (e) {
      if (e instanceof ApiError)
        return c.json(
          { error: e.message },
          e.status === 422 ? 400 : (e.status as 400)
        );
      throw e;
    }
  }
  ...
  app.get("/sep6/transactions", (c) => {
    ...
    if (q("asset_code") !== stellar.assetCode)
      return sepError(c, 400, `unsupported asset_code '${q("asset_code")}'`);
    const customer = c.get("sepCustomer");
    const kind = q("kind");
    if (kind && kind !== "deposit" && kind !== "withdrawal")
      return sepError(c, 400, "'kind' must be deposit or withdrawal");
    const limit = Math.min(200, Math.max(1, Number(q("limit") ?? 50) || 50));
    const noOlderThan = q("no_older_than") ?? null;
    if (noOlderThan && Number.isNaN(new Date(noOlderThan).getTime()))
      return sepError(c, 400, "'no_older_than' must be an ISO 8601 timestamp");
    let pagingBefore: string | null = null;
    if (q("paging_id")) {
      const ref = db
        .prepare(
          "SELECT created_at FROM sep_transactions WHERE id = ? AND customer_id = ?"
        )
        .get(q("paging_id")!, customer.id) as
        { created_at: string } | undefined;
      if (!ref)
        return sepError(
          c,
          400,
          "'paging_id' does not refer to one of your transactions"
        );
    ...
    const rows = db
      ...
      .all(
        ...
        kind ?? null,
        kind ?? null,
        noOlderThan ? new Date(noOlderThan).toISOString() : null,
        noOlderThan ? new Date(noOlderThan).toISOString() : null,
        pagingBefore,
        pagingBefore,
        limit
      ) as unknown as SepTransactionRow[];
    return c.json({
      transactions: rows.map((r) =>
        sepTransactionOut(cfg, stellar, bundle(db, r))
      ),
    });
  });
  ...
  app.get("/sep6/transaction", (c) => {
    ...
    if (q("id"))
      row = db
        .prepare(
          "SELECT * FROM sep_transactions WHERE id = ? AND customer_id = ?"
        )
        .get(q("id")!, customer.id) as unknown as SepTransactionRow | undefined;
      ...
      row = db
        ...
        .get(
          customer.id,
          q("stellar_transaction_id")!,
          q("stellar_transaction_id")!
        ) as unknown as SepTransactionRow | undefined;
      ...
      return sepError(
        c,
        400,
        "provide 'id', 'stellar_transaction_id' or 'external_transaction_id'"
      );
    if (!row) return sepError(c, 404, "transaction not found");
    return c.json({
      transaction: sepTransactionOut(cfg, stellar, bundle(db, row)),
    });
  });
  ...
  function txPage(b: SepBundle) {
    ...
    const held = !economicActionsEnabled(cfg) && status.startsWith("pending");
    if (held) t.message = POLICY_PENDING_MESSAGE;
    const rows = Object.entries(t)
      .filter(
        ([k, v]) =>
          ...
          v !== undefined &&
          typeof v !== "object" &&
          !["more_info_url", "refunded"].includes(k)
      )
      .map(([k, v]) => {
        const val =
          k === "stellar_transaction_id"
            ? `<a class="mono" target="_blank" rel="noopener" href="https://stellar.expert/explorer/testnet/tx/${esc(v)}">${esc(v)}</a>`
    ...
    let action = "";
    if (held) {
      action = `<section class="panel"><h3>Transfers disabled</h3><p>${esc(POLICY_PENDING_MESSAGE)}</p><p>Do not send funds for this transaction.</p></section>`;
    } else if (
      b.tx.kind === "deposit" &&
      ...
      b.tx.kind === "withdrawal" &&
      status === "pending_user_transfer_start"
    ) {
      const uri = stellarPayUri({
        destination: b.offramp!.deposit_address,
        assetCode: stellar.assetCode,
        assetIssuer: stellar.assetIssuer,
        memoId: b.offramp!.memo_id,
        amount: b.offramp!.expected_usdc,
        msg: "TR Mock Anchor withdrawal",
      });
    ...
    const refresh = status.startsWith("pending")
      ? '<meta http-equiv="refresh" content="5">'
      : "";
    return page(
      `${b.tx.kind} ${b.tx.id}`,
      `<h1 style="font-size:24px">${esc(b.tx.kind)} - <span class="status ${esc(status)}">${esc(status)}</span></h1>${action}<section class="panel" style="margin-top:14px"><dl class="kv">${rows}</dl></section>`,
      refresh
    );
  }
