...
import { economicActionsEnabled } from "./anchor-policy.js";
...
export function createWorkers(
  ...
  async function settleOnrampsOnce(): Promise<number> {
    // A policy hold must not enter the retry/refund path or settle legacy rows.
    if (!economicActionsEnabled(deps.cfg)) return 0;
    const rows = db
      .prepare(
        "SELECT * FROM onramps WHERE status = 'pending' ORDER BY created_at ASC LIMIT 20"
      )
    ...
    for (const row of rows) {
      ...
      if (stroops > treasury) {
        db.prepare(
          "UPDATE onramps SET pending_reason = 'treasury_low', updated_at = ? WHERE id = ?"
        ).run(nowIso(), row.id);
        log.warn(
          `onramp ${row.id} waiting: treasury has ${fmtUsdc(treasury)} USDC, needs ${row.amount_usdc}`
        );
        continue;
      }
      try {
        ...
        if (res.settlement === "awaiting_trust") {
          ...
          if (row.pending_reason !== "awaiting_trust") {
            db.prepare(
              "UPDATE onramps SET pending_reason = 'awaiting_trust', updated_at = ? WHERE id = ?"
            ).run(nowIso(), row.id);
            log.info(
              `onramp ${row.id} awaiting a USDC trustline on ${row.destination_address}`
            );
          }
          continue;
        }
        ...
        settled++;
        log.info(
          `onramp ${row.id} completed: ${row.amount_usdc} USDC -> ${row.destination_address} via ${res.settlement} (${res.txHash})`
        );
      } catch (e) {
        ...
        if (retryable && attempts < MAX_SEND_ATTEMPTS) {
          db.prepare(
            "UPDATE onramps SET attempts = ?, pending_reason = ?, updated_at = ? WHERE id = ?"
          ).run(
            attempts,
            `retrying: ${err.message}`.slice(0, 200),
            nowIso(),
            row.id
          );
          log.warn(
            `onramp ${row.id} attempt ${attempts} failed (will retry): ${err.message}`
          );
        } else {
          const ts = nowIso();
          const failed: OnrampRow = {
            ...row,
            status: "failed",
            pending_reason: null,
          ...
          tx(db, () => {
            db.prepare(
              "UPDATE onramps SET status = 'failed', pending_reason = NULL, failure_reason = ?, attempts = ?, updated_at = ? WHERE id = ?"
            ).run(failed.failure_reason, attempts, ts, row.id);
            // Give the TRY back.
            applyLedger(
              db,
              row.customer_id,
              "TRY",
              BigInt(row.amount_try.replace(".", "")),
              "onramp_refund",
              row.id
            );
  ...
  async function handleIncoming(p: IncomingPayment): Promise<void> {
    ...
    const already = db
      .prepare("SELECT id FROM offramps WHERE tx_hash = ?")
      .get(p.txHash);
    if (already) return;

    const offramp = key
      ? (db.prepare("SELECT * FROM offramps WHERE memo_id = ?").get(key) as
          OfframpRow | undefined)
      : undefined;
    if (!offramp || offramp.status !== "awaiting_deposit") {
      ...
      log.warn(
        `unmatched USDC deposit ${fmtUsdc(p.amountStroops)} tx ${p.txHash}: ${reason}`
      );
      return;
    }
    ...
    tx(db, () => {
      ...
      emitEvent(
        ...
        offrampOut(received, stellar.assetCode, stellar.assetIssuer)
      );
      ...
      db.prepare(
        ...
        completed.received_usdc,
        completed.amount_try,
        completed.rate,
        completed.repriced,
        completed.tx_hash,
        completed.from_address,
        payoutId,
        ts,
        ts,
        offramp.id
      ...
      emitEvent(
        ...
        offramp.partner_id,
        "offramp.completed",
        offrampOut(completed, stellar.assetCode, stellar.assetIssuer)
      );
    });
    log.info(
      `offramp ${offramp.id} completed: received ${fmtUsdc(p.amountStroops)} USDC -> ${fmtTry(kurus)} TRY${repriced ? " (repriced)" : ""} tx ${p.txHash}`
    );
  }

  async function watchOfframpsOnce(): Promise<number> {
    // Keep the ingestion cursor intact while conversion and payout are disabled.
    if (!economicActionsEnabled(deps.cfg)) return 0;
    ...
    for (const p of payments) {
      try {
        ...
        log.error(
          `failed to process incoming payment ${p.txHash}: ${(e as Error).message}`
        );
      }
    }
  ...
  async function deliverWebhooksOnce(): Promise<number> {
    const due = db
      .prepare(
        "SELECT id, webhook_id, event_type, payload, attempts FROM webhook_deliveries WHERE status = 'pending' AND next_attempt_at <= ? ORDER BY created_at ASC LIMIT 20"
      )
    ...
    for (const d of due) {
      const hook = db
        ...
        .get(d.webhook_id) as WebhookRow | undefined;
      if (!hook) {
        db.prepare(
          "UPDATE webhook_deliveries SET status = 'cancelled' WHERE id = ?"
        ).run(d.id);
        continue;
      }
      ...
      if (!error) {
        ...
        ).run(attempts, statusCode, nowIso(), d.id);
        delivered++;
      } else if (attempts > WEBHOOK_BACKOFF_SECONDS.length) {
        db.prepare(
          "UPDATE webhook_deliveries SET status = 'failed', attempts = ?, last_status_code = ?, last_error = ? WHERE id = ?"
        ).run(attempts, statusCode, error, d.id);
        log.warn(
          `webhook ${hook.id} delivery ${d.id} gave up after ${attempts} attempts: ${error}`
        );
      } else {
        const wait = WEBHOOK_BACKOFF_SECONDS[attempts - 1]!;
        db.prepare(
          "UPDATE webhook_deliveries SET attempts = ?, last_status_code = ?, last_error = ?, next_attempt_at = ? WHERE id = ?"
        ).run(
          attempts,
          statusCode,
          error,
  ...
  async function sepCallbacksOnce(): Promise<number> {
    const rows = db
      .prepare(
        "SELECT * FROM sep_transactions WHERE on_change_callback IS NOT NULL AND on_change_callback != 'postMessage'"
      )
    ...
    for (const row of rows) {
      ...
      const body = JSON.stringify({
        transaction: sepTransactionOut(deps.cfg, stellar, b),
      });
      const t = Math.floor(Date.now() / 1000);
      const host = safeHost(row.on_change_callback!);
      const sig = Buffer.from(
        sep.signingKeypair.sign(Buffer.from(`${t}.${host}.${body}`))
      ).toString("base64");
      try {
      ...
      db.prepare(
        "UPDATE sep_transactions SET last_callback_status = ? WHERE id = ?"
      ).run(status, row.id);
      if (status === "completed" || status === "error") {
        db.prepare(
          "UPDATE sep_transactions SET completed_at = COALESCE(completed_at, ?) WHERE id = ?"
        ).run(nowIso(), row.id);
      }
    }
    return sent;
