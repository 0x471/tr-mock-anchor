...
import { assertEconomicActionsEnabled } from "../anchor-policy.js";
...
export async function resolveRate(
  ...
  if (quote)
    return {
      rateMicro: parseRate(quote.rate),
      midMicro: parseRate(quote.mid_rate),
      quoteId: quote.id,
    };
...
export function createOnramp(
  ...
  if (customer.kyc_status !== "approved")
    throw unprocessable(
      "kyc_not_approved",
      `Customer KYC status is ${customer.kyc_status}`
    );
  if (!isStellarAddress(args.destination))
    throw badRequest(
      "invalid_destination_address",
      "destination must be a Stellar account (G...) or muxed (M...) address. Contract addresses (C...) are not supported: USDC is a classic asset paid via a classic payment, which cannot target a contract."
    );
  if (Number(cfg.minOnrampTry) > 0 && args.kurus < parseTry(cfg.minOnrampTry))
    throw unprocessable(
      "below_minimum",
      `Minimum on-ramp is ${cfg.minOnrampTry} TRY`
    );
  if (Number(cfg.maxOnrampTry) > 0 && args.kurus > parseTry(cfg.maxOnrampTry))
    throw unprocessable(
      "above_maximum",
      `Maximum on-ramp is ${cfg.maxOnrampTry} TRY`
    );
  const stroops = tryToUsdc(args.kurus, args.rate.rateMicro);
  if (stroops <= 0n)
    throw unprocessable("amount_too_small", "Amount rounds to zero USDC");
  ...
  const row: OnrampRow = {
    ...
    claimable_balance_supported:
      args.claimableBalanceSupported === false ? 0 : 1,
    status: "pending",
    pending_reason: null,
    settlement: null,
    tx_hash: null,
  ...
  if (row.quote_id)
    db.prepare("UPDATE quotes SET consumed_by = ? WHERE id = ?").run(
      row.id,
      row.quote_id
    );
...
export function createOfframp(
  ...
  if (
    ...
    args.expected !== null &&
    args.expected < parseUsdc(cfg.minOfframpUsdc)
  ) {
    throw unprocessable(
      "below_minimum",
      `Minimum off-ramp is ${cfg.minOfframpUsdc} USDC`
    );
  }
  let payoutIban: string | null = args.payoutIban
    ? normalizeIban(args.payoutIban)
    : customer.iban;
  if (payoutIban && !isValidTrIban(payoutIban))
    throw badRequest(
      "invalid_iban",
      "payout_iban must be a valid Turkish IBAN"
    );
  if (args.autoPayout && !payoutIban) {
    throw unprocessable(
      "missing_iban",
      "auto_payout requires an IBAN: set customer.iban or pass payout_iban (or set auto_payout=false to keep TRY on the balance)"
    );
  }
  ...
  emitEvent(
    ...
    customer.partner_id,
    "offramp.created",
    offrampOut(row, stellar.assetCode, stellar.assetIssuer)
  );
  return row;
}
