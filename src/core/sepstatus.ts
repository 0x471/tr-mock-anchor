...
export function loadSepTx(db: DB, id: string): SepBundle | null {
  const tx = db
    .prepare("SELECT * FROM sep_transactions WHERE id = ?")
    .get(id) as unknown as SepTransactionRow | undefined;
  return tx ? bundle(db, tx) : null;
}

export function bundle(db: DB, tx: SepTransactionRow): SepBundle {
  const onramp = tx.onramp_id
    ? ((db
        .prepare("SELECT * FROM onramps WHERE id = ?")
        .get(tx.onramp_id) as unknown as OnrampRow | undefined) ?? null)
    : null;
  const offramp = tx.offramp_id
    ? ((db
        .prepare("SELECT * FROM offramps WHERE id = ?")
        .get(tx.offramp_id) as unknown as OfframpRow | undefined) ?? null)
    : null;
  const payout = offramp?.payout_id
    ? ((db
        .prepare("SELECT * FROM payouts WHERE id = ?")
        .get(offramp.payout_id) as unknown as PayoutRow | undefined) ?? null)
    : null;
...
export function sepStatusOf(b: SepBundle): SepStatus {
  ...
  if (b.tx.kind === "deposit") {
    ...
    if (b.onramp.status === "pending") {
      ...
      return b.onramp.pending_reason?.startsWith("retrying")
        ? "pending_stellar"
        : "pending_anchor";
    }
  ...
  if (b.offramp.status === "awaiting_deposit")
...
function sepMessage(b: SepBundle, status: SepStatus): string | null {
  ...
  if (b.tx.kind === "deposit") {
    if (status === "pending_user_transfer_start")
      return "Waiting for the TRY bank transfer. Sandbox: simulate it from more_info_url.";
    if (status === "pending_anchor")
      return b.onramp?.pending_reason === "treasury_low"
        ? "Treasury is low on USDC; the payout will settle once it is refilled."
        : "TRY received; paying USDC on Stellar.";
    if (status === "pending_trust")
      return `Add a USDC trustline to ${b.tx.account}; the anchor pays the USDC once the trustline exists.`;
    if (status === "pending_stellar") return b.onramp?.pending_reason ?? null;
    if (status === "error")
      return (
        b.onramp?.failure_reason ??
        "On-ramp failed; TRY was refunded to the balance."
      );
  } else {
    if (status === "pending_user_transfer_start")
      return `Send ${b.offramp?.expected_usdc ?? "any amount of"} USDC to ${b.offramp?.deposit_address} with memo (id) ${b.offramp?.memo_id}.`;
    if (status === "completed")
      return `TRY paid to ${b.payout?.iban ?? b.offramp?.payout_iban} via FAST (simulated).`;
  }
  return null;
}
...
export function sepTransactionOut(
  cfg: Config,
  stellar: StellarGateway,
  b: SepBundle
) {
  const storedStatus = sepStatusOf(b);
  const held =
    !economicActionsEnabled(cfg) &&
    (storedStatus.startsWith("pending_") || storedStatus === "incomplete");
  const status = held ? "incomplete" : storedStatus;
  ...
  const base = {
    ...
    status,
    status_eta:
      status === "pending_anchor" || status === "pending_stellar" ? 5 : null,
    more_info_url: `${cfg.publicUrl}/sep6/tx/${b.tx.id}`,
    message: held ? POLICY_PENDING_MESSAGE : sepMessage(b, status),
    ...
    user_action_required_by:
      status === "pending_user_transfer_start" && b.tx.kind === "withdrawal"
        ? (b.offramp?.rate_locked_until ?? null)
        : null,
    quote_id:
      b.tx.quote_id ?? b.onramp?.quote_id ?? b.offramp?.quote_id ?? null,
    refunded: false,
  ...
  if (b.tx.kind === "deposit") {
    const o = b.onramp;
    const fee = o
      ? feeTry(parseTry(o.amount_try), parseUsdc(o.amount_usdc), o.mid_rate)
      : null;
    ...
    return {
        ...
        fee === null
          ...
          : {
              ...
              details: [
                {
                  name: "spread",
                  description: `${cfg.spreadBps} bps over the USD/TRY mid rate`,
                  amount: fmtTry(fee),
                },
              ],
            },
      from: null,
      to: b.tx.account,
      deposit_memo: held ? null : b.tx.memo,
      deposit_memo_type: held ? null : b.tx.memo_type,
      ...
      instructions:
        !held && b.tx.reference
          ? depositInstructions(cfg, b.tx.reference)
          : null,
      refunded,
      refunds:
        refunded && o
          ? {
              amount_refunded: o.amount_try,
              amount_fee: "0.00",
              payments: [
                {
                  id: o.id,
  ...
  const fee =
    f && received && f.amount_try
      ? feeTry(parseTry(f.amount_try), parseUsdc(received), f.mid_rate)
      : null;
  return {
    ...base,
      ...
      fee === null
        ...
        : {
            ...
            details: [
              {
                name: "spread",
                description: `${cfg.spreadBps} bps under the USD/TRY mid rate`,
                amount: fmtTry(fee),
              },
            ],
          },
    ...
    withdraw_anchor_account: held
      ? null
      : (f?.deposit_address ?? stellar.treasuryPublicKey),
    withdraw_memo: held ? null : (f?.memo_id ?? null),
    withdraw_memo_type: held ? null : "id",
...
export function depositInstructions(cfg: Config, reference: string) {
  return {
    bank_name: {
      value: cfg.bankName,
      description: "Bank holding the anchor account",
    },
    bank_account_number: {
      value: cfg.anchorIban,
      description: `IBAN to send TRY to (account holder: ${cfg.accountHolder})`,
    },
    external_transfer_memo: {
      value: reference,
      description:
        "Write this reference in the transfer description (aciklama). It routes the money to your account.",
    },
  };
}
