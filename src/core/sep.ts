...
import { economicActionsEnabled, type AnchorMode } from "../anchor-policy.js";
...
export function ensureSepPartner(db: DB): PartnerRow {
  const existing = db
    ...
    .get(SEP_PARTNER_ID) as unknown as PartnerRow | undefined;
  ...
  return db
    .prepare("SELECT * FROM partners WHERE id = ?")
    .get(SEP_PARTNER_ID) as unknown as PartnerRow;
}
 ...
 * customer under the SEP partner. Only explicit legacy mode auto-approves simulated KYC.
 * A deterministic sandbox IBAN stands in for the user's bank account
 * so simulated payouts have somewhere to go.
 */
export function ensureSepCustomer(
  db: DB,
  sub: string,
  anchorMode: AnchorMode = "zkpassport"
): CustomerRow {
  const found = db
    .prepare("SELECT * FROM customers WHERE partner_id = ? AND external_id = ?")
    .get(SEP_PARTNER_ID, sub) as unknown as CustomerRow | undefined;
  ...
  db.prepare(
    ...
    null,
    null,
    makeTrIban("00099", digits),
    economicActionsEnabled({ anchorMode }) ? "approved" : "pending",
  ...
  return db
    ...
    .get(id) as unknown as CustomerRow;
}

/** SEP-38 asset identifiers. */
export const TRY_ASSET = "iso4217:TRY";
export const usdcAsset = (code: string, issuer: string) =>
  `stellar:${code}:${issuer}`;
