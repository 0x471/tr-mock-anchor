...
export type SepEnv = AppEnv & {
  Variables: { sepSub: string; sepCustomer: CustomerRow };
};

/** SEP-10 bearer auth. Failures use the SEP-6 error shape `{"type":"authentication_required"}` (HTTP 403). */
export function sepJwtAuth(
  deps: Deps,
  sep: SepContext
): MiddlewareHandler<SepEnv> {
  return async (c, next) => {
    ...
    c.set(
      "sepCustomer",
      ensureSepCustomer(deps.db, claims.sub, deps.cfg.anchorMode)
    );
    await next();
  };
}
...
export const sepError = (c: Context, status: 400 | 404, message: string) =>
