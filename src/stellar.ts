...
export interface StellarGateway {
  ...
  sendUsdc(args: {
    ...
    amountStroops: bigint;
    memo?: string;
    allowClaimableBalance?: boolean;
  }): Promise<SendResult>;
  /** Incoming USDC payments to the treasury after `cursor` (Horizon paging token). */
  incomingUsdc(
    cursor: string | undefined
  ): Promise<{ payments: IncomingPayment[]; cursor: string | undefined }>;
...
export function baseAccountOf(addr: string): string {
  return StrKey.isValidMed25519PublicKey(addr)
    ? MuxedAccount.fromAddress(addr, "0").baseAccount().accountId()
    : addr;
}
...
export function createLiveGateway(cfg: Config): StellarGateway {
  ...
  async function loadOrNull(accountId: string) {
    try {
      ...
      throw new StellarError(
        `horizon loadAccount failed: ${(e as Error).message}`,
        true
      );
    }
  }
  ...
  async function submit(
    ...
    try {
      ...
      throw new StellarError(
        `rpc getAccount failed: ${(e as Error).message}`,
        true
      );
    }
    const tx = build(
    ...
    try {
      ...
      throw new StellarError(
        `rpc sendTransaction failed: ${(e as Error).message}`,
        true
      );
    }
    if (sent.status === "ERROR") {
      ...
      throw new StellarError(
        `submit rejected: ${xdrCode(sent.errorResult)}`,
        true,
        sent.errorResult
      );
    }
    ...
    throw new StellarError(
      `tx ${sent.hash} submitted but not confirmed within timeout`,
      false
    );
  }
  ...
  async function claimableBalanceIdOf(
    txHash: string
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      const effects = await server
        ...
        .limit(50)
        .call();
      const created = effects.records.find(
  ...
  return {
    ...
    async treasuryUsdcBalance() {
      ...
      const bal = acct.balances.find((b) => isUsdc(b as never)) as
...
export function createFakeGateway(
  ...
  return {
    ...
    async sendUsdc({
      ...
      if (noTrustline && !allowClaimableBalance)
        return { settlement: "awaiting_trust" };
      if (amountStroops > balance)
        throw new StellarError("submit failed: op_underfunded", false);
      balance -= amountStroops;
      const txHash =
        `fake${(++seq).toString().padStart(4, "0")}${"0".repeat(56)}`.slice(
          0,
          64
        );
      const result: SendResult = noTrustline
        ? {
            settlement: "claimable_balance",
            txHash,
            claimableBalanceId: `00000000${txHash}`,
          }
    ...
    simulateIncoming(p) {
      ...
      const payment: IncomingPayment = {
        txHash:
          `fakein${(++seq).toString().padStart(4, "0")}${"0".repeat(56)}`.slice(
            0,
            64
          ),
