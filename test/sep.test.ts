...
async function authenticate(kp: Keypair, memo?: string) {
  const ch = await get(
    `/auth?account=${kp.publicKey()}${memo ? `&memo=${memo}` : ""}`,
    {}
  );
  ...
  return {
    status: res.status,
    ...(body as { token?: string; error?: string }),
  };
}
...
describe("SEP door", () => {
  ...
  it("SEP-10: rejects unsigned, wrongly signed and tampered challenges", async () => {
    ...
    const unsigned = await app.request("/auth", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ transaction: ch.json.transaction }),
    });
    ...
    const wrong = await app.request("/auth", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ transaction: tx.toXdr() }),
    });
    expect(wrong.status).toBe(400);
    const garbage = await app.request("/auth", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ transaction: "AAAA" }),
    });
    expect(garbage.status).toBe(400);
    const missing = await app.request("/auth", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: "{}",
    });
  ...
  it("SEP-10: funded accounts are verified against their signers and threshold", async () => {
    ...
    stellar.registerAccount(funded.publicKey(), {
      signers: [
        ...
        { key: signer2.publicKey(), weight: 1, type: "ed25519_public_key" },
      ],
      thresholds: { low_threshold: 1, med_threshold: 2, high_threshold: 2 },
    });
    const ch = await get(`/auth?account=${funded.publicKey()}`, {});
    const one = TransactionBuilder.fromXdr(
      ch.json.transaction,
      Networks.TESTNET
    );
    one.sign(funded);
    const tooWeak = await app.request("/auth", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ transaction: one.toXdr() }),
    });
    expect(tooWeak.status).toBe(400);
    one.sign(signer2);
    const ok = await app.request("/auth", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ transaction: one.toXdr() }),
    });
  ...
  it("SEP-12: new wallet user is NEEDS_INFO with only optional fields, PUT accepts without personal data", async () => {
    ...
    expect(
      Object.values(before.json.fields).every((f: any) => f.optional === true)
    ).toBe(true);
    const put = await app.request("/sep12/customer", {
      method: "PUT",
      headers: { ...auth(), "content-type": "application/json" },
      body: JSON.stringify({ email_address: "w@example.com" }),
    });
    ...
    const put2 = await app.request("/sep12/customer", {
      method: "PUT",
      headers: { ...auth(), "content-type": "application/json" },
      body: JSON.stringify({
        tax_id: "10000000146",
        bank_account_number: "TR33 0006 1005 1978 6457 8413 26",
      }),
    });
    expect(put2.status).toBe(202);
    const row = db
      .prepare("SELECT tckn, iban FROM customers WHERE id = ?")
      .get(id) as { tckn: string | null; iban: string };
    expect(row.tckn).toBeNull();
    expect(row.iban).toBe("TR330006100519786457841326");
    const badIban = await app.request("/sep12/customer", {
      method: "PUT",
      headers: { ...auth(), "content-type": "application/json" },
      body: JSON.stringify({ bank_account_number: "TR000" }),
    });
  ...
  it("SEP-12: memos distinguish customers on the same account, DELETE forgets", async () => {
    ...
    const reg = async (t: string) =>
      (
        (await (
          await app.request("/sep12/customer", {
            method: "PUT",
            headers: {
    ...
    const ca = await get("/sep12/customer", {
      authorization: `Bearer ${a.token}`,
    });
    expect(ca.json.id).toBe(ida);
    expect(ca.json.status).toBe("ACCEPTED");
    const fresh = await get("/sep12/customer", {
      authorization: `Bearer ${(await authenticate(Keypair.random())).token}`,
    });
    expect(fresh.json.id).toBeUndefined(); // no data collected yet -> no id, NEEDS_INFO
    const del = await app.request(`/sep12/customer/${wallet.publicKey()}`, {
      method: "DELETE",
      headers: auth(),
    });
  ...
  it("SEP-6 deposit: returns bank instructions, then the simulated transfer triggers a real on-ramp", async () => {
    ...
    const badAsset = await get(
      `/sep6/deposit?asset_code=XYZ&account=${wallet.publicKey()}`
    );
    expect(badAsset.status).toBe(400);
    const dep = await get(
      `/sep6/deposit?asset_code=USDC&account=${wallet.publicKey()}&funding_method=bank_account&amount=200.00&claimable_balance_supported=true`
    );
    expect(dep.status).toBe(200);
    expect(dep.json.id).toMatch(/^sep_/);
    expect(dep.json.how).toContain("TRMA-");
    expect(dep.json.instructions.bank_account_number.value).toMatch(
      /^TR\d{24}$/
    );
    expect(dep.json.instructions.external_transfer_memo.value).toMatch(
      /^TRMA-/
    );
    ...
    expect(t.json.transaction.more_info_url).toBe(
      `http://localhost:8787/sep6/tx/${depositId}`
    );
    ...
    const fund = await app.request(
      `/sep6/tx/${depositId}/simulate-bank-transfer`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
    ...
    const byHash = await get(
      `/sep6/transaction?stellar_transaction_id=${t.json.transaction.stellar_transaction_id}`
    );
  ...
  it("SEP-6 deposit honors claimable_balance_supported=false: holds in pending_trust until a trustline exists", async () => {
    ...
    const dep = await get(
      `/sep6/deposit?asset_code=USDC&account=${holder.publicKey()}&funding_method=bank_account&amount=200.00`,
      h
    );
    expect(dep.status).toBe(200);
    const id = dep.json.id;
    await app.request(`/sep6/tx/${id}/simulate-bank-transfer`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ amount: "200.00" }),
    });
  ...
  it("SEP-6 withdraw: returns the treasury account + memo, completes when the payment lands", async () => {
    const w = await get(
      `/sep6/withdraw?asset_code=USDC&type=bank_account&amount=2.5`
    );
    ...
    expect(t.json.transaction.withdraw_anchor_account).toBe(
      stellar.treasuryPublicKey
    );
  ...
  it("SEP-6 transactions: list newest first, honours limit, kind, no_older_than and paging_id", async () => {
    ...
    expect(
      new Date(all.json.transactions[0].started_at) >=
        new Date(all.json.transactions[1].started_at)
    ).toBe(true);
    const one = await get("/sep6/transactions?asset_code=USDC&limit=1");
    expect(one.json.transactions.length).toBe(1);
    const deposits = await get(
      "/sep6/transactions?asset_code=USDC&kind=deposit"
    );
    expect(deposits.json.transactions.map((t: any) => t.id)).toEqual([
      depositId,
    ]);
    const paged = await get(
      `/sep6/transactions?asset_code=USDC&paging_id=${all.json.transactions[0].id}`
    );
    expect(paged.json.transactions.map((t: any) => t.id)).toEqual([depositId]);
    const recent = await get(
      `/sep6/transactions?asset_code=USDC&no_older_than=${encodeURIComponent(all.json.transactions[0].started_at)}`
    );
  ...
  it("SEP-38: prices, price and firm quotes with consistent formulas", async () => {
    ...
    const prices = await get(
      "/sep38/prices?sell_asset=iso4217:TRY&sell_amount=100",
      {}
    );
    expect(prices.json.buy_assets[0].asset).toBe(
      `stellar:USDC:${stellar.assetIssuer}`
    );
    const price = await get(
      `/sep38/price?sell_asset=iso4217:TRY&buy_asset=stellar:USDC:${stellar.assetIssuer}&sell_amount=100&context=sep6`,
      {}
    );
    ...
    expect(
      Math.round((Number(p.sell_amount) / Number(p.total_price)) * 100) / 100
    ).toBe(
      Number(p.buy_amount) > 0
        ? Math.round(Number(p.buy_amount) * 100) / 100
        : 0
    );
    expect(
      Math.round((Number(p.sell_amount) - Number(p.fee.total)) * 100) / 100
    ).toBe(Math.round(Number(p.price) * Number(p.buy_amount) * 100) / 100);
    expect(p.fee.asset).toBe("iso4217:TRY");

    const q = await app.request("/sep38/quote", {
      method: "POST",
      headers: { ...auth(), "content-type": "application/json" },
      body: JSON.stringify({
        sell_asset: `stellar:USDC:${stellar.assetIssuer}`,
    ...
    const noauth = await app.request("/sep38/quote", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: "{}",
    });
    ...
    const wx = await get(
      `/sep6/withdraw-exchange?source_asset=USDC&destination_asset=iso4217:TRY&amount=10&quote_id=${quote.id}&funding_method=bank_account`
    );
    ...
    const dq = await app.request("/sep38/quote", {
      method: "POST",
      headers: { ...auth(), "content-type": "application/json" },
      body: JSON.stringify({
        sell_asset: "iso4217:TRY",
        buy_asset: `stellar:USDC:${stellar.assetIssuer}`,
        sell_amount: "100",
        context: "sep6",
      }),
    });
    ...
    const dx = await get(
      `/sep6/deposit-exchange?destination_asset=USDC&source_asset=iso4217:TRY&amount=100&quote_id=${dquote.id}&type=bank_account`
    );
  ...
  it("SEP-6 on_change_callback: posts signed status changes", async () => {
    ...
    try {
      const dep = await get(
        `/sep6/deposit?asset_code=USDC&account=${wallet.publicKey()}&on_change_callback=${encodeURIComponent(url)}`
      );
      expect(dep.status).toBe(200);
      ...
      const ok = sep.signingKeypair.verify(
        Buffer.from(
          `${m[1]}.127.0.0.1:${(server.address() as { port: number }).port}.${d.body}`
        ),
        Buffer.from(m[2]!, "base64")
      );
      expect(ok).toBe(true);
      expect(JSON.parse(d.body).transaction.status).toBe(
        "pending_user_transfer_start"
      );
      await app.request(`/sep6/tx/${dep.json.id}/simulate-bank-transfer`, {
        method: "POST",
      });
      expect(await workers.sepCallbacksOnce()).toBe(1);
      expect(JSON.parse(received[1]!.body).transaction.status).toBe(
        "pending_anchor"
      );
    } finally {
      server.close();
